...
module.exports = {
  ...
  aliases    : ['verrouiller'],
  category   : 'moderation',
  description: 'Verrouille un salon pour empêcher les membres d\'écrire.',
  usage      : ';lock [#salon]',
  cooldown   : 3,
  guildOnly  : true,
  ownerOnly  : false,
  ...
  async execute(message, args) {
    ...
    loadingCt.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${e('ani_loading')} Verrouillage en cours...`));
    ...
    try {
      ...
      await channel.permissionOverwrites.edit(
        ...
        { SendMessages: false },
        { reason: `Lock par ${message.author.tag}` }
      );

      const ts = Math.floor(Date.now() / 1000);
      const ct = new ContainerBuilder().setAccentColor(0xFF0000);
      ct.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${e('ani_diamond')} **Salon verrouillé**`));
      ...
      ct.addTextDisplayComponents(new TextDisplayBuilder().setContent(
        `${e('ui_lock')} Salon : ${channel}\n` +
        `${e('ui_user')} Verrouillé par : ${message.author}\n` +
        `${e('ui_pin')} Date : <t:${ts}:R>`
      ));
      ct.addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
      ct.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${e('btn_success')} Les membres ne peuvent plus écrire dans ce salon.`));
      ...
      errCt.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${e('btn_error')} **Erreur** — Je n'ai pas la permission de verrouiller ce salon.`));
