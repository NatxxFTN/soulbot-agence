...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (!role) return message.reply({ embeds: [E.error('Rôle manquant', 'Mentionne le rôle à retirer après le membre.')] });
    ...
    if (role.position >= botMember.roles.highest.position) {
      return message.reply({ embeds: [E.error('Hiérarchie insuffisante', 'Ce rôle est supérieur ou égal au mien — impossible de le retirer.')] });
    }

    if (!target.roles.cache.has(role.id)) {
      return message.reply({ embeds: [E.warning('Rôle absent', `${target} ne possède pas le rôle **${role.name}**.`)] });
    }
    ...
    try {
      ...
      return message.reply({ embeds: [E.error('Erreur API', `Impossible de retirer le rôle : ${err.message}`)] });
    }

    STMT_LOG.run(message.guild.id, 'REMOVEROLE', target.id, target.user.tag, message.author.id, message.author.tag, `Rôle: ${role.name} — ${reason}`);

    return message.channel.send({
      embeds: [
        E.success('Rôle retiré')
          .addFields(
