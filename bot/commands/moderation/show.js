...
module.exports = {
  name       : 'show',
  aliases    : ['afficher', 'visible'],
  category   : 'moderation',
  description: 'Rend un salon visible à nouveau pour les membres.',
  usage      : ';show [#salon]',
  cooldown   : 3,
  guildOnly  : true,
  ownerOnly  : false,
  ...
  async execute(message, args) {
    ...
    loadingCt.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${e('ani_loading')} Affichage en cours...`));
    ...
    try {
      ...
      await channel.permissionOverwrites.edit(
        ...
        { ViewChannel: null },
        { reason: `Show par ${message.author.tag}` }
      );

      const ts = Math.floor(Date.now() / 1000);
      const ct = new ContainerBuilder().setAccentColor(0xFF0000);
      ct.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${e('ani_diamond')} **Salon visible**`));
      ...
      ct.addTextDisplayComponents(new TextDisplayBuilder().setContent(
        ...
        `${e('ui_user')} Affiché par : ${message.author}\n` +
        `${e('ui_pin')} Date : <t:${ts}:R>`
      ));
      ct.addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small));
      ct.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${e('btn_success')} Le salon est à nouveau visible pour les membres.`));
      ...
      errCt.addTextDisplayComponents(new TextDisplayBuilder().setContent(`${e('btn_error')} **Erreur** — Je n'ai pas la permission d'afficher ce salon.`));
