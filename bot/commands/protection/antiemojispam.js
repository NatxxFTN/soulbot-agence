...
const FEATURE = 'antiemojispam';
const META = {
  label: 'Anti-Emoji spam',
  emoji: 'ui_smiley',
  description: "Bloque les messages avec trop d'emojis.",
...
module.exports = {
  name       : FEATURE,
  aliases    : ['aemojispam', 'aemoji'],
  category   : 'protection',
  description: META.description,
  usage      : ';antiemojispam [on|off|action <type>|threshold <5-30>]',
  cooldown   : 3,
  guildOnly  : true,
  ...
  async execute(message, args) {
    ...
    if (['on', 'enable', 'activer'].includes(sub)) {
      ...
      return plain(message, `${e('btn_success')} **${META.label}** activé. Seuil : **${cfg.threshold}** emojis · Action : \`${cfg.action}\`.`);
    }
    ...
    if (sub === 'threshold' || sub === 'seuil') {
      ...
      if (Number.isNaN(n) || n < 5 || n > 30) {
        return plain(message, `${e('btn_error')} Seuil invalide. Plage : **5-30**.`);
      }
      storage.setConfig(message.guild.id, FEATURE, { threshold: n });
      return plain(message, `${e('btn_success')} Seuil **${META.label}** → **${n}** emojis.`);
    }
