...
const FEATURE = 'antimention';
const META = {
  label: 'Anti-Mention',
  emoji: 'ui_user',
  description: 'Anti-spam de mentions users/rôles.',
  ...
  async execute(message, args) {
    ...
    if (sub === 'threshold' || sub === 'seuil') {
      ...
      if (Number.isNaN(n) || n < 2 || n > 25) {
        return plain(message, `${e('btn_error')} Seuil invalide. Plage : **2-25**.`);
      }
      storage.setConfig(message.guild.id, FEATURE, { threshold: n });
      return plain(message, `${e('btn_success')} Seuil **${META.label}** → **${n}** mentions.`);
    }
