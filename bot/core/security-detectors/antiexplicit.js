...
// ── Détecteur antiexplicit — mots marqueurs (defaults neutres + custom) ─────
...
module.exports = {
  ...
  async check(message, config) {
    ...
    const all = [...DEFAULT_EXPLICIT_WORDS, ...customWords.map(w => String(w).toLowerCase())];

    for (const w of all) {
      if (!w) continue;
      const regex = new RegExp(`\\b${escapeRegex(w)}\\b`, 'i');
      if (regex.test(content)) {
        return { triggered: true, reason: `Contenu explicite détecté (mot "${w}")` };
      }
    }
